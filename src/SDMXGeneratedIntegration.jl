...
function demonstrate_generated_parsing()
    ...
    if dim_node !== nothing && attr_node !== nothing && measure_node !== nothing
        ...
        println("  ID: $(dim_data.dimension_id)")
        println("  Position: $(dim_data.position)")
        println("  Concept: $(dim_data.concept_id)")
        println("  Codelist: $(dim_data.codelist_id)")
        
        # Demonstrate attribute parsing
        println("\\n🏷️  Attribute Element:")
        attr_data = extract_sdmx_element(AttributeElement, attr_node)
        println("  ID: $(attr_data.attribute_id)")
        println("  Assignment: $(attr_data.assignment_status)")
        println("  Attachment: $(attr_data.attachment_level)")
        
        # Demonstrate measure parsing
        println("\\n📈 Measure Element:")
        measure_data = extract_sdmx_element(MeasureElement, measure_node)
        println("  ID: $(measure_data.measure_id)")
        println("  Data Type: $(measure_data.data_type)")
        println("  Decimals: $(measure_data.decimals)")
